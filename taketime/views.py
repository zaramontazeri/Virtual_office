...
class ReserveTimeAPIView(views.APIView):
    def post(self , request):
        ...
        if time_id==0:
            ...
            time = models.Time.objects.filter(id = time_id, is_active=True).first() 
            if time :
                time_ser=serializers.TimeSerializer(time, data= {"is_active":False , "user":user},partial=True)
                # time_ser.is_active=False
                # time_ser.user=user
                # data=time_ser.data
                # time_ser = serializers.TimeSerializer(time, data=time_ser.data)
                time_ser.is_valid()
                time_ser.save()
            ...
            # print(serializer)
                ...
                # raise  ValidationError(code = 'time_list_empty' , detail= 'there is no time inj list')
